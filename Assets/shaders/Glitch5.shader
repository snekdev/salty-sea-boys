...
Shader "Unlit/Glitch5"
{
	...
	//			float bbb = (i.wpos.x + i.wpos.y + i.wpos.z + _SinScale * sin(_Dilation * _Time.y));
				//float bbb = sin(_Time.y) + fmod(tan(abs(i.wpos.x)), .3) / fmod(sin(i.wpos.y), .2) + fmod(sin(abs(sin(i.wpos.z))), .3);
			//	float bbb = sin(_Time.y) + sin(i.wpos.x) + sin(i.wpos.y) +sin(i.wpos.z); 
			float bbb = sin(_Time.y) - sin(i.wpos.x) + pow (sin(i.wpos.y),3) + sin(i.wpos.z); 



				col = lerp(_Color1, _Color2, 2 * abs(fmod( bbb , 1)) - .5 );
