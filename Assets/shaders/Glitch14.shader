...
Shader "Unlit/coral3"
{
	...
	//	_SecondaryTex("_SecondaryTex", 2D) = "white" {}
		//_Range("Range", Range(0,20)) = 0.0
			...
			_SinScale2("_SinScale2", Float) = 0.0
	...
	SubShader
	{
		...
		//	float	_Range;
			...
			float	_ModConst;
			float	_SinScale;			float	_SinScale2;
		...
		//	float	_Percent;
			...
			{ 
				...
				v.vertex.xz += _SinScale *sin( fmod(aaa1, _ModConst));
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				col = lerp(_Color1, _Color2, fmod( .3 *( i.wpos.x + i.wpos.y +  i.wpos.z + _SinScale2 * sin(_Dilation * _Time.y)), 1));
