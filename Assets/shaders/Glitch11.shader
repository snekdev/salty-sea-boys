...
Shader "Unlit/Glitch11"
{
	...
	//	_SecondaryTex("_SecondaryTex", 2D) = "white" {}
		//_Range("Range", Range(0,20)) = 0.0
			...
			_Dilation2("_Dilation", Float) = 0.0
	...
	SubShader
	{
		...
		//	float	_Percent;
			...
			{ 
				...
				v.vertex.xyz +=  v.normal *(abs(sin(_Dilation2 *_Time.y)) *sin(fmod(aaa1, _ModConst)) - .07);
			...
			float4 aaa = mul(unity_ObjectToWorld, i.vertex);
			i.wpos *= _WorldSpaceScale;
			col = lerp(_Color1, _Color2, fmod(.3 *(i.wpos.y + _SinScale * sin(_Time.y)), 1));
			UNITY_APPLY_FOG(i.fogCoord, col);
			return col;
/*
				fixed4 col = tex2D(_MainTex, i.uv);
